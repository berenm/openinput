...
extern DECLSPEC sint OICALL oi_events_peep(oi_event *evts, sint num);

// Add an event to the tail of the queue (number_added)
extern DECLSPEC sint OICALL oi_events_add(oi_event *evts, sint num);

// Pump all device to transfer events into queue (n/a)
extern DECLSPEC void OICALL oi_events_pump();

// Poll events (more_pending)
extern DECLSPEC sint OICALL oi_events_poll(oi_event *evt);

// Wait for an event (n/a)
extern DECLSPEC void OICALL oi_events_wait(oi_event *evt);

// Set event type filter mask (n/a)
extern DECLSPEC void OICALL oi_events_setmask(uint mask);

// Get event type filter mask (event_mask)
extern DECLSPEC uint OICALL oi_events_getmask();

/* ******************************************************************** */

// Send events for down-state keys (errorcode)
extern DECLSPEC sint OICALL oi_key_repeat(sint delay, sint interval);

// Get key state table and set num to number of elements (pointer)
extern DECLSPEC uchar OICALL *oi_key_keystate(uchar index, sint *num);

// Return modifier mask (modifier_mask)
extern DECLSPEC uint OICALL oi_key_modstate(uchar index);
...
extern DECLSPEC sint OICALL oi_mouse_absolute(uchar index, sint *x, sint *y);

// Get relative motion of mouse (button_mask)
extern DECLSPEC sint OICALL oi_mouse_relative(uchar index, sint *x, sint *y);

// Warp mouse cursor position (errorcode)
extern DECLSPEC sint OICALL oi_mouse_warp(uchar index, sint x, sint y);
...
// Get mouse-id given name (oi_mouse)
extern DECLSPEC uint OICALL oi_joy_getcode(char *name);
...
extern DECLSPEC sint OICALL oi_app_focus();
...
extern DECLSPEC sint OICALL oi_action_install(oi_actionmap *map, sint num);

// Check/validate single actionmap structure (errorcode)
extern DECLSPEC sint OICALL oi_action_validate(oi_actionmap *map);

// Get action state table and set num to number of elements (pointer)
extern DECLSPEC uchar OICALL *oi_action_actionstate(sint *num);
